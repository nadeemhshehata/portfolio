...
public class BST<T extends Comparable<T>> {
	...
	if (source.getHeight() == target.getHeight()) {
	    ArrayList<CountedEntity<T>> sourceData = source.inOrder();
	    ArrayList<CountedEntity<T>> targetData = target.inOrder();

	    if (sourceData.size() != targetData.size()) {
		return false;
	    }
	    for (int i = 0; i < sourceData.size(); i++) {
		if (sourceData.get(i).compareTo(targetData.get(i)) != 0) {
		    return false;
		}
	    }
	    return true;
	...
	// BST property checks: min < node < max, plus correct height
	...
	if (result < 0) {
		...
		// Both children exist: find the smallest node in the right subtree
	...
	return root == null; // Assuming 'root' is the reference to the root node of the tree
    }
