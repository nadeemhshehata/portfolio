...
public class A04Main {
	...
	final ArrayList<CountedEntity<Character>> array = tree.inOrder();
	int totalCount = 0;

	for (final CountedEntity<Character> data : array) {
	    totalCount += data.getCount();
	}
	System.out.println("Char    Count Percent");

	for (final CountedEntity<Character> countedEntity : array) {
	    final int count = countedEntity.getCount();
	    final double percent = (double) countedEntity.getCount() / totalCount * 100;
	    System.out.format("%4s %,8d %7.2f%n", countedEntity.getEntity(), count, percent);
	}
	return;
    }
	...
	for (final char letter : string.toCharArray()) {
	    final CountedEntity<Character> data = new CountedEntity<Character>(Character.toUpperCase(letter));
	    tree.insert(data);
	}
	return;
    }
	...
	while (fileScan.hasNextLine()) {
		...
		if (Character.isLetter(c)) {
		    final CountedEntity<Character> key = new CountedEntity<Character>(Character.toUpperCase(c));
		    tree.retrieve(key);
		}
	    }
	}
	...
	for (Integer i : testData) {
	    CountedEntity<Integer> data = new CountedEntity<>(i);
	    source.insert(data);
	}
	...
	CountedEntity<Integer> key = new CountedEntity<>(3);
