...
public abstract class SingleLink<T> implements Iterable<T> {
	...
	if (this.front == null) {
	    // If the current list is empty, simply take over the source list
	    this.front = source.front;
	    this.rear = source.rear;
	} else {
	    // Link the rear of 'this' list to the front of 'source'
	...
	// If source is now empty, update its rear pointer
	...
	// Insert node at the front of this list
	node.setNext(this.front);
	this.front = node;

	// If this list was empty, update its rear pointer
	if (this.rear == null) {
	    this.rear = node;
	}
