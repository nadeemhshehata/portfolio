...
class Hash_Set:
    ...
    def insert(self, value):
        ...
        # Re-insert elements from the old table into the new table
        for slot in temp:
            while len(slot) > 0:  # Only proceed if the slot is not empty
                value = slot.remove_front()
                self.insert(value)
    ...
    def find(self, key):
        ...
        # your code here
        slot = self._find_slot(key)
        return key if key in slot else None
    ...
    def __eq__(self, target):
        ...
        """
        # your code here
        if self._capacity != target._capacity or self._count != target._count:
            return False
